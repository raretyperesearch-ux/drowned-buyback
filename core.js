        ...
        for (var j = 0; j < accounts.value.length; j++) {
          ...
          if (acc.account.data.parsed.info.mint === mintAddress) {
            return {
              ...
              programId: programId.toString() // ADDED: Return which program this token uses
            };
          }
        }
      } catch (e) {
...
class SolanaTrackerSwap {
  ...
  async buyWithSol(wallet, tokenMint, solAmount) {
    ...
    // Deserialize and sign the transaction (v0 versioned transaction)
    var { VersionedTransaction } = require('@solana/web3.js');
    var txBuffer = Buffer.from(data.txn, 'base64');
    var tx = VersionedTransaction.deserialize(txBuffer);
    tx.sign([wallet]);
...
class TokenBurner {
  ...
  async burn(wallet, mintAddress) {
    ...
    var programIds = [TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID];
    var connection = this.connection;
    var detectedProgramId = null; // FIX: Track which program the token uses

    // FIX: Find the token account and remember which program it uses
    for (var i = 0; i < programIds.length; i++) {
      ...
      try {
        ...
        for (var j = 0; j < accounts.value.length; j++) {
          ...
          if (acc.account.data.parsed.info.mint === mintAddress) {
            ...
            detectedProgramId = pid; // FIX: Remember which program this token uses
            break;
          }
        }
      } catch (e) {
    ...
    // FIX: Use the detected program ID (critical for Token-2022 tokens)
    if (!detectedProgramId) {
      detectedProgramId = TOKEN_PROGRAM_ID; // Fallback to standard program
    }
    ...
    console.log('   Using token program: ' + (detectedProgramId.equals(TOKEN_2022_PROGRAM_ID) ? 'Token-2022' : 'Token Program'));

    // FIX: Pass the correct program ID when getting the burn token account address
    var burnTokenAccount = await getAssociatedTokenAddress(
      mintPubkey, 
      BURN_ADDRESS, 
      true,  // allowOwnerOffCurve
      detectedProgramId  // FIX: Use the detected program ID
    );
    ...
    if (!burnAccountInfo) {
      ...
      // FIX: Pass the correct program ID when creating the burn token account
      tx.add(
        createAssociatedTokenAccountInstruction(
          wallet.publicKey,      // payer
          burnTokenAccount,      // associatedToken
          BURN_ADDRESS,          // owner
          mintPubkey,            // mint
          detectedProgramId      // FIX: Use the detected program ID
        )
    ...
    // FIX: Pass the correct program ID in the transfer instruction
    tx.add(
