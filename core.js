...
class PumpPortalSwap {
  ...
  async buyWithSol(wallet, tokenMint, solAmount, slippage = 25) {
    ...
    // Fetch tx result with retry loop (RPC might not have indexed it yet)
    let txResult = null;
    let retries = 0;
    
    while (!txResult && retries < 3) {
      txResult = await this.connection.getTransaction(signature, {
        maxSupportedTransactionVersion: 0,
        commitment: 'confirmed'
      });
      if (!txResult) {
        console.log(`   ⏳ Waiting for RPC to index tx (attempt ${retries + 1}/3)...`);
    ...
    if (!txResult) {
      throw new Error('Transaction not found after confirmation - RPC indexing issue');
    }
    ...
    console.log(`   ✅ Buy CONFIRMED and SUCCEEDED`);
  ...
  async sellForSol(wallet, tokenMint, tokenAmount, slippage = 25) {
    ...
    // Fetch tx result with retry loop
    let txResult = null;
    let retries = 0;
