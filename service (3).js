  ...
  async executeBuybackBurn(tokenMint) {
    ...
    if (projectBuybackSol >= 0.01) {
      try {
        ...
        // Wait for RPC to index the new balance (increased from 3s to 5s)
        ...
        // If no balance, wait a bit more and retry once
        if (!tokenBalance || tokenBalance.amount <= 0) {
          console.log(`   ⏳ Balance not found, waiting 3 more seconds...`);
        ...
        if (tokenBalance && tokenBalance.amount > 0) {
          ...
          console.log(`   ⚠️ No tokens found to burn - balance is 0`);
        }
      } catch (e) {
    ...
    if (platformFeeSol >= 0.005 && this.config.platformTokenMint) {
      try {
        ...
        let platformTokenBalance = await this.monitor.getTokenBalance(
          walletAddress, 
          this.config.platformTokenMint
        );
        ...
        if (!platformTokenBalance || platformTokenBalance.amount <= 0) {
          console.log(`   ⏳ Balance not found, waiting 3 more seconds...`);
          await new Promise(r => setTimeout(r, 3000));
          platformTokenBalance = await this.monitor.getTokenBalance(
            walletAddress, 
            this.config.platformTokenMint
          );
        }
        
        console.log(`   💰 Platform token balance: ${platformTokenBalance ? platformTokenBalance.amount : 0}`);
        ...
        if (platformTokenBalance && platformTokenBalance.amount > 0) {
          ...
          console.log(`   ⚠️ No platform tokens found to burn - balance is 0`);
        }
      } catch (e) {
