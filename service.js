...
class BuybackBurnService {
  constructor(config) {
    ...
    this.jupiter = new JupiterSwap(config.heliusApiKey);
   ...
   * Returns a unique deposit wallet address
   */
  async registerProject(tokenMint, tokenName, tokenTicker, creatorWallet) {
    ...
    if (this.webhookManager) {
      try {
        ...
        // Don't fail registration, webhook can be synced later
      }
    }
  ...
  /**
   * Execute buyback-burn for a single project
   * Splits fees: platform gets X%, project token gets rest
   */
  async executeBuybackBurn(tokenMint) {
    ...
    if (projectBuybackSol >= 0.01) {
      try {
        ...
        const buyResult = await this.jupiter.buyWithSol(wallet, tokenMint, projectBuybackSol);
    ...
    if (platformFeeSol >= 0.005 && this.config.platformTokenMint) {
      try {
        // Transfer platform fee to platform burn wallet
        const platformWallet = deriveWallet(this.config.seedPhrase, this.config.platformBurnWalletIndex);
        
        console.log(`   📈 Buying platform token...`);
        
        // If platform fee is in the same wallet, use it directly
        // Otherwise transfer first
        const buyResult = await this.jupiter.buyWithSol(
          wallet, 
  ...
  // ============================================================================
  // BATCH PROCESSING - RUN ALL PROJECTS
  // ============================================================================
   ...
   * Run this on startup or if webhooks get out of sync
   */
  async syncWebhooks() {
    if (!this.webhookManager) {
  ...
  async getDashboardData() {
    ...
    const totalTokensBurned = projects.reduce(
      (sum, p) => sum + parseFloat(p.total_tokens_burned || 0), 0
    );
