...
class BuybackBurnService {
  constructor(config) {
    this.config = {
      ...
      solanaTrackerApiKey: config.solanaTrackerApiKey || process.env.SOLANA_TRACKER_API_KEY
    };
    ...
    // Primary: Solana Tracker (if API key available)
    if (this.config.solanaTrackerApiKey) {
      this.solanaTrackerSwap = new SolanaTrackerSwap(this.config.solanaTrackerApiKey);
    }
    
    // Fallback: PumpPortal
    this.pumpPortalSwap = new PumpPortalSwap(config.heliusApiKey);
  ...
  async swapWithFallback(wallet, tokenMint, solAmount) {
    // Try Solana Tracker first (if available)
    if (this.solanaTrackerSwap) {
      try {
        console.log('   Trying SolanaTracker...');
        var result = await this.solanaTrackerSwap.buyWithSol(wallet, tokenMint, solAmount);
        return result;
      } catch (e) {
        console.log('   SolanaTracker failed: ' + e.message);
        console.log('   Falling back to PumpPortal...');
      }
    }
    
    // Fallback to PumpPortal
    return await this.pumpPortalSwap.buyWithSol(wallet, tokenMint, solAmount);
  }
  ...
  async executeBuybackBurn(tokenMint) {
    ...
    if (projectBuybackSol >= 0.01) {
      try {
        ...
        var buyResult = await this.swapWithFallback(wallet, tokenMint, projectBuybackSol);
    ...
    if (platformFeeSol >= 0.005 && this.config.platformTokenMint) {
      try {
        ...
        var platformBuy = await this.swapWithFallback(wallet, this.config.platformTokenMint, platformFeeSol);
